...
## 下一步建议
**V2.4 已完成（v2.4.1）**，演进路线详见 [ROADMAP.md](ROADMAP.md) 顶部「计划中 V2.5 → V2.7」：
- **先做**：用真实 key 跑评测台基线 `cd backend && python -m evals.run_eval --repeat 3`（复审 M-1），并用真实样本替换 `backend/evals/gold/*.jsonl` 合成脚手架
- **V2.5 闭环**（已批准，下一站）：首次运行预置案例库 + top-N 案例注入 analyze/critique grounding（改前后必跑评测台）+ 结构化课程
- V2.6 质量：前端测试 + agent 输出评测进 CI + 缓存 + 成本/延迟遥测
- V2.7 触达：运行时可配 API base URL + 桌面打包/托管 demo
- 复审遗留（需 key/Node/运行时）：M-1 真实校准 · R-1 前端测试 · R-4 Vision 直评校准 · R-6 运行时 API URL · R-7 遥测
