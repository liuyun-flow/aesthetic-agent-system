...
## 下一步建议（V2.5 信心，进行中）
**V2.4 已完成（v2.4.1）**；当前在做 **V2.5 信心**（详见 [docs/V2.5_PLAN.md](docs/V2.5_PLAN.md)）。进度：
- ✅ **N1 CI**（GitHub Actions：后端 pytest + 前端 Vitest + build）· ✅ **N2 缓存（视觉描述）+ 遥测**（llm_usage 表 + /system/usage + 设置页）· ✅ **N3 前端组件测试**（Vitest+RTL）
- ⏳ **N4 Playwright E2E** · N5 评测进 on-release CI · N6 收尾 v2.5.0
- 之后：**V2.7 触达**（运行时 API URL + 桌面打包） · **~V3.0 闭环**（[docs/V3.0_PLAN.md](docs/V3.0_PLAN.md)：预置案例库 + grounding + 课程）
- 独立待办（需 key）：M-1 真实校准基线（`python -m evals.run_eval --repeat 3` + 替换真实金标准）· R-4 Vision 直评校准 · R-6 运行时 API URL
- **V2.7 触达**：运行时可配 API base URL + 桌面打包/托管 demo
- **~V3.0 闭环**，详见 [docs/V3.0_PLAN.md](docs/V3.0_PLAN.md)：预置案例库 + top-N grounding（改前后必跑评测台）+ 结构化课程
- 独立待办（需 key/Node/运行时）：M-1 真实校准基线（`python -m evals.run_eval --repeat 3` + 替换真实金标准）· R-4 Vision 直评校准 · R-6 运行时 API URL
