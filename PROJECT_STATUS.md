...
## Current Version
**v1.0.0（2026-06-27）首个正式稳定版** — 版本线归一：此前 V1.0–V2.6 为 1.0 前的开发迭代编号，本次将正式发布版定为 v1.0.0（首个稳定版）。功能集即 V2.6 全部能力（Editorial Atelier UI + 信任度量 + 信心质量基线）。下方「版本演进总览」与各版本「本次变更」保留为历史开发记录。
...
V2.5.0 是「信心（质量与可靠性）」版：在扩大受众前锁住质量，让改动不能悄悄回退。详见 [docs/V2.5_PLAN.md](docs/V2.5_PLAN.md)。

| 类别 | 变更 |
|------|------|
| CI（N1） | **New** `.github/workflows/ci.yml`：push/PR 跑后端 pytest + 前端 Vitest + build + Playwright E2E |
| 缓存（N2） | 视觉描述按 provider+model 复用（`uploaded_images.vision_model`，`?refresh=true` 强制）；评分不缓存 |
| 遥测（N2） | `llm_usage` 表 + 透明 `wrap_client`（计 token+延迟）+ `GET /system/usage` + 设置页用量面板 |
| 前端测试（N3） | Vitest + RTL（ResultCard / TaskForm）；`vitest.config.mts`（esbuild 自动 JSX） |
| E2E（N4） | Playwright smoke（启动整栈，核心路由渲染，无需 key）；CI 有独立 e2e job |
| on-release 评测（N5） | **New** `.github/workflows/evals.yml`：发布时跑评测台（需 secret），`--check` 门槛 gate 发布 |
| 迁移 | `_migrate_v2_5`：+vision_model 列、+llm_usage 表（幂等） |
| 版本号 | main.py / data_io.py / tests → v2.5.0 |

### 修改的文件（V2.5.0）
| File | Change |
|------|--------|
| `.github/workflows/ci.yml` | **New** — backend pytest + frontend Vitest/build + e2e |
| `.github/workflows/evals.yml` | **New** — on-release 校准评测 |
| `backend/app/db/models.py` `database.py` | +`LlmUsage` 表、+`vision_model` 列、`_migrate_v2_5` |
| `backend/app/services/telemetry.py` | **New** — record_usage / get_usage_summary |
| `backend/app/llm/tracked_client.py` | **New** — 透明计量 wrap_client |
| `backend/app/llm/deepseek_client.py` `vision/openai_adapter.py` | 包 wrap_client |
| `backend/app/main.py` | +`/system/usage`、describe 缓存、`_current_vision_signature`、version 2.5.0 |
| `backend/app/services/session_service.py` `schemas/responses.py` | +vision_model / UsageSummary / ImageDescribeResponse.cached |
| `backend/evals/run_eval.py` | +`--check` / `_passes_thresholds` |
| `backend/app/tests/test_evals.py` | +阈值门槛单测 |
| `frontend/` | Vitest+RTL（vitest.config.mts / setup / 2 test 文件）、Playwright（config + e2e/smoke）、设置页用量面板、package.json scripts |
...
| 类别 | 变更 |
|------|------|
| 评测/校准台 | **New** `backend/evals/`（dev-only，.dockerignore 排除）：金标准（合成脚手架待替换）+ run_eval.py（成对判对率/Spearman/分档单调性）+ PROMPT_VERSION 钉死 + `--dry-run` |
| 存储维度分 | training_records +`ai_dimension_scores`/`ai_overall_score`/`eval_prompt_version`；`_migrate_v2_4` 自动加列；critique 保存 critic 8 维分（0-100），无额外 LLM |
| 维度评估 | `/assessment/dimensions` 有存储分→聚合 8 维（critic 6 + 价格感/商业适配）；无分→回落关键词法（旧数据/无 key 不变） |
| critic 扩维 | DimensionScores +price_perception/commercial_fit（可选，向后兼容）；prompt 请求 8 维 |
| Vision 直评 | 可选实验性：`SCORING_VISION_DIRECT` + OpenAI key 时 critique 直接看图打分，默认关、失败回落 |
| 测试 | 220 passed（+2 维度聚合 +2 vision-direct flag）；旧 assessment 测试因回落路径不变而全绿 |
| 版本号 | main.py / data_io.py / tests → v2.4.0 |

### 修改的文件（V2.4.0）
| File | Change |
|------|--------|
| `backend/evals/` | **New** — run_eval.py / gold/*.jsonl / README / reports（dev-only） |
| `backend/app/agents/design_knowledge.py` | +PROMPT_VERSION |
| `backend/app/agents/critic.py` | prompt 扩 8 维；+VisionCriticAgent |
| `backend/app/schemas/responses.py` | DimensionScores +2 可选商业维度 |
| `backend/app/db/models.py` | training_records +3 列 |
| `backend/app/db/database.py` | +_migrate_v2_4 |
| `backend/app/services/session_service.py` | save_record +3 参数 |
| `backend/app/services/assessment.py` | compute_dimension_scores 聚合派发 + _aggregate/_keyword 分离 + WORK_DIMENSIONS |
| `backend/app/main.py` | +_extract_dimension_kwargs +_vision_direct_enabled/_maybe_vision_critic；critique 接线；version 2.4.0 |
| `backend/app/services/data_io.py` | EXPORT_VERSION v2.4.0 |
| `backend/app/tests/test_assessment.py` `test_api.py` `test_preflight.py` | +4 测试 + 版本断言 |
| `.env.example` | +DEEPSEEK_TIMEOUT_SECONDS +SCORING_VISION_DIRECT |
| `.dockerignore` | +backend/evals/ |
...
1. 开发者在受限网络下向 GitHub push 需本机代理（端口随代理软件变化，**不写入仓库**）；最终用户本地部署与使用**无需任何代理**。本机 git 已改为只用全局代理配置，避免本地 override 指向失效端口
2. Git Bash 下 curl 无法连接 127.0.0.1（用浏览器或 Python + ProxyHandler({}) 替代）
3. HTTP_PROXY 环境变量可能导致 Python urllib 本地连接失败（`gh` 也受影响，需确保 env 指向存活的代理端口）
...
- .bat 静态检查测试
- Playwright E2E（V2.5 N4，进行中；组件测试已在 N3 加入）

## 下一步建议
**V2.5 信心已完成（v2.5.0）。** 下一站候选：
- **V2.7 触达**：运行时可配 API base URL（Next.js rewrite，解决 known issue #5）+ 桌面打包/托管 demo（API key 是真门槛）
- **~V3.0 闭环**（[docs/V3.0_PLAN.md](docs/V3.0_PLAN.md)）：预置案例库 + top-N grounding（改前后必跑评测台）+ 结构化课程
- **独立待办（需 key）**：M-1 真实校准基线 —— 给仓库加 `DEEPSEEK_API_KEY` secret + 用真实样本替换 `backend/evals/gold/*.jsonl`，然后 on-release `evals.yml` 才有意义；R-4 Vision 直评校准
