...
## v1.0.0 (2026-06-27) — 首个正式稳定版
- **版本线归一**：此前 V1.0–V2.6 为 1.0 前的开发迭代编号；本次将正式发布版定为 **v1.0.0**（首个稳定版）。功能集即 V2.6 全部能力。
- 版本号同步至所有站点：`main.py`、`data_io.py`（EXPORT_VERSION）、`test_api.py` / `test_preflight.py` 断言（导入兼容仍接受旧 `v1.`/`v2.` 包）。
- 评测台转向（`backend/evals/STRATEGY.md`）：开发者「金标准」→ 极简「评分器健全性自检」+ 面向用户的个人样本库。
- **已知局限（诚实记录）**：图片→描述→评分链路中，视觉描述会「洗掉」视觉层面的缺陷（首次自检 win-rate 0.0 即源于此），故评分对「视觉糟糕但被描述得体」的作品不敏感；需后续「视觉直评」(R-4) 解决。
- 247 后端测试全绿。
