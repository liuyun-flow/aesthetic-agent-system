...
2. **本机已装 Node v24.16.0**（2026-06-15 via winget）；前端 `npm run build` 和 `npm run test`（Vitest）本机直接可跑，无需 Docker。Node 不在 PowerShell 默认 PATH，命令前置 `$env:Path = "C:\Program Files\nodejs;$env:Path"`
