...
export default function SessionList({ refreshKey, onRetrain, onAddToLibrary }: Props) {
  ...
  const renderValue = (key: string, value: unknown) => {
    if (isEmptyValue(value)) return <span className="text-muted">暂无</span>;
    if (LIST_FIELDS.has(key)) {
      const items = toDisplayList(value);
      if (items.length === 0) return <span className="text-muted">暂无</span>;
      return (
        <ul className="list-inside list-disc space-y-0.5">
          {items.map((item, i) => (
            <li key={i} className="text-ink-soft">{item}</li>
          ))}
        </ul>
      );
    }
    ...
    return text
      ? <span className="text-ink-soft whitespace-pre-wrap">{text}</span>
      : <span className="text-muted">暂无</span>;
  };

  if (loading && sessions.length === 0) {
    return <section className="text-sm text-muted">{t.sessions.loading}</section>;
  }
  if (error) {
    return <section className="text-sm text-red-500">{t.sessions.error}</section>;
  }
  if (sessions.length === 0) {
    return <section className="text-sm text-muted">{t.sessions.noSessions}</section>;
  }
    ...
    <>
        ...
        <div className="space-y-2">
          {sessions.map((s) => (
            <div
              ...
              className="flex cursor-pointer items-start gap-3 rounded border bg-surface px-3 py-2 text-sm
                         hover:border-accent-soft hover:bg-accent-wash transition"
              title="点击查看详情"
            >
              <span className={`mt-0.5 rounded px-1.5 py-0.5 text-xs font-medium ${
                s.record_type === "analyze" ? "bg-accent-wash text-accent"
              ...
              <span className="flex-1 truncate text-ink-soft">{s.work_description}</span>
              {s.judgment_gap_summary && (
                <span className="text-xs text-accent">⚡</span>
              )}
              <span className="text-xs text-accent whitespace-nowrap">查看详情 →</span>
      ...
      {selectedId !== null && (
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-10 bg-black/40"
             onClick={closeDetail}>
          <div className="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-surface p-6 shadow-xl"
               onClick={(e) => e.stopPropagation()}>
            <button onClick={closeDetail}
              className="absolute right-4 top-4 text-muted hover:text-ink-soft text-xl leading-none">
              ✕
            </button>

            {detailLoading && (
              <p className="text-sm text-muted">正在加载历史详情...</p>
            )}
            {detailError && (
            ...
            {detail && (
              <div className="space-y-5 text-sm">
                <div>
                  <h3 className="text-lg font-semibold text-ink mb-2">训练详情</h3>
                  <div className="flex gap-3 text-xs text-muted">
                    <span className={`rounded px-2 py-0.5 font-medium ${
                      detail.record_type === "analyze" ? "bg-accent-wash text-accent"
                ...
                {/* Work description */}
                <Section title="作品描述">
                  <p className="whitespace-pre-wrap text-ink-soft">{detail.work_description}</p>
                </Section>
                  ...
                  <Section title="用户初步判断">
                    <div className="space-y-2">
                      {(["user_score", "user_strengths", "user_weaknesses", "user_priority_fixes", "user_target_audience", "user_price_band"] as const).map((key) => {
                        ...
                        return (
                          <div key={key}>
                            <span className="text-xs font-medium text-muted">{FIELD_LABELS[key] ?? key}</span>
                  ...
                  <Section title="AI 分析结果">
                    <div className="space-y-2">
                      {(["ai_score", "ai_main_problems", "ai_priority_fixes"] as const).map((key) => {
                        ...
                        return (
                          <div key={key}>
                            <span className="text-xs font-medium text-muted">{FIELD_LABELS[key]}</span>
                            <div className="mt-0.5">{renderValue(key, value)}</div>
                          </div>
                        );
                      })}
                      ...
                      {!detail.ai_main_problems && !detail.ai_priority_fixes && (
                        <pre className="whitespace-pre-wrap rounded bg-surface-2 p-2 text-xs text-ink-soft max-h-60 overflow-y-auto">
                ...
                {(detail.judgment_gap_summary || detail.training_focus_tags) && (
                  <Section title="判断差异分析">
                    {detail.judgment_gap_summary && (
                      <div className="mb-2">
                        <span className="text-xs font-medium text-muted">{FIELD_LABELS.judgment_gap_summary}</span>
                        <p className="text-ink-soft whitespace-pre-wrap">{detail.judgment_gap_summary}</p>
                      </div>
                    )}
                    {!isEmptyValue(detail.training_focus_tags) && (
                      <div>
                        <span className="text-xs font-medium text-muted">{FIELD_LABELS.training_focus_tags}</span>
                          ...
                          return (
                            ...
                            >
                              <div className="mb-1 flex items-center gap-2">
                                <span className="text-xs text-muted">[{directionId}]</span>
                                <span className="text-sm font-semibold text-ink-soft">
                                  {valueToText(direction.title) || `方向 ${index + 1}`}
                                </span>
                                {isSelected && (
                                  <span className="rounded bg-accent px-1.5 py-0.5 text-[11px] text-white">
                                    已选择
                                  </span>
                                )}
                              </div>
                              <div className="space-y-1">
                                {Object.entries(DIRECTION_DETAIL_LABELS).map(([key, label]) => {
                                  ...
                                  if (!text) return null;
                                  return (
                                    <p key={key} className="text-xs text-ink-soft">
                          ...
                          <div key={key}>
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <button
                                ...
                                className="rounded border border-line px-2 py-0.5 text-xs text-muted hover:bg-surface-2"
                              >
                                {copiedKey === `hist-${key}` ? "已复制" : "复制"}
                              </button>
                            </div>
                            <pre className="whitespace-pre-wrap rounded bg-surface-2 p-2 text-xs text-ink-soft max-h-32 overflow-y-auto">
                              {text}
                            </pre>
                          </div>
                        );
                ...
                <div className="flex gap-2 pt-2 border-t">
                  {onRetrain && (
                    <button
                      ...
                      className="rounded bg-accent px-3 py-1 text-xs font-medium text-white hover:bg-accent-deep"
                  ...
                  {detail.result_json && (
                    <button onClick={() => handleCopy(JSON.stringify(detail.result_json, null, 2), "result_json")}
                      className="rounded border border-line px-3 py-1 text-xs text-ink-soft hover:bg-surface-2">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted border-b pb-1">
        {title}
      </h4>
      {children}
    </div>
