  ...
  return (
    <section className="space-y-4">
      ...
      <div className="rounded border bg-surface p-4 shadow-soft">
        <h2 className="mb-3 text-base font-semibold text-ink">今日训练</h2>
        {today ? (
          <>
            <p className="mb-2 text-sm">
              主题：<span className="font-semibold text-accent">{String(today.theme)}</span>
            </p>
      ...
      {stats && (
        <div className="rounded border bg-surface p-4 shadow-soft">
          <h2 className="mb-2 text-base font-semibold text-ink">训练统计</h2>
      ...
      {lastSessionId && (
        <div className="rounded border bg-surface p-4 shadow-soft">
          <h2 className="mb-2 text-base font-semibold text-ink">完成训练记录</h2>
          ...
          <div className="flex gap-2 items-center mb-2">
            ...
            <button onClick={handleComplete} disabled={completing}
              className="rounded bg-green-600 px-4 py-1 text-xs text-white hover:bg-green-700 disabled:bg-muted">
              {completing ? "保存中…" : "标记完成"}
            </button>
      ...
      <button onClick={handleWeeklyReview} disabled={reviewLoading}
        className="rounded bg-ink px-4 py-2 text-sm text-white hover:bg-ink disabled:bg-muted">
      ...
      {review && (
        <div className="rounded border bg-surface-2 p-4">
          <h3 className="mb-2 text-sm font-semibold text-ink">本周复盘</h3>
          {(["summary", "common_misjudgments", "progress_points", "recurring_issues"] as const).map((k) => (
            (review[k] as string) && <p key={k} className="mb-2 text-sm text-ink-soft">{String(review[k])}</p>
          ))}
          ...
          {Array.isArray(review.next_week_tasks) && (review.next_week_tasks as string[]).length > 0 && (
            <ul className="list-inside list-disc mt-1 text-sm text-ink-soft">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded bg-surface-2 px-2 py-1 text-center">
